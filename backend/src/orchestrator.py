...
def _to_landmark(feature: overpass.OsmFeature) -> Landmark:
    ...
    # Needs GOOGLE_MAPS_KEY; otherwise None -> the frontend shows a category icon.
    image_url = None
    if config.has_maps():
        image_url = (
            "https://maps.googleapis.com/maps/api/streetview"
            f"?size=600x240&location={feature.lat},{feature.lng}&fov=80&source=outdoor&key={config.GOOGLE_MAPS_KEY}"
        )
    return Landmark(
        name=name,
